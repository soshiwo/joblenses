    ...
    return [
        {
            ...
            "opts": [
                ...
                {"t": "Present the report but verbally disclose the 2% variance during the meeting, promising an updated data addendum.", "s": [5, 5, 10, 10, 0]},
                {"t": "Present the report as-is to secure the executive bonus vote, then quietly deploy a hotfix for all future reporting.", "s": [0, 0, 5, 5, 0]},
                {"t": "Mad-dash to write a quick Python script to patch the compiled data in memory, risking an untested, undocumented fix.", "s": [0, 10, 0, 0, 5]}
            ]
        },
        {
            "id": "s2",
            ...
            "opts": [
                ...
                {"t": "Create a highly aggregated, anonymized version of the dashboard that shows macro trends without exposing individual identities.", "s": [0, 10, 5, 10, 5]},
                {"t": "Build exactly what the CRO asked for; they explicitly own the legal risk, and your team desperately needs the budget.", "s": [0, 5, 0, 5, 0]},
                {"t": "Collaborate with the CRO's chief of staff to understand the root business problem and offer a safer, alternative sales-enablement metric.", "s": [5, 0, 10, 0, 0]}
            ]
        },
        {
            "id": "s3",
            ...
            "opts": [
                {"t": "Deploy the pipeline immediately to drastically improve output, telling the team they need to upskill to Python or be left behind.", "s": [5, 5, 0, 10, 0]},
                {"t": "Delete your Python work and help the team incrementally improve the existing Excel macros so everyone stays comfortable and aligned.", "s": [0, 0, 10, 0, 10]},
                {"t": "Propose a phased rollout where you run the Python pipeline in parallel while running weekly lunch-and-learns to teach the team Python.", "s": [10, 10, 5, 0, 5]},
                {"t": "Keep using the Python script secretly to finish your own work faster, leaving the team to their manual processes.", "s": [0, 0, 0, 5, 0]}
            ]
        },
        {
            "id": "s4",
            ...
            "opts": [
                {"t": "Issue a final warning or fire them. Toxic behavior destroys the team culture, even if it hurts short-term business results.", "s": [10, 5, 10, 0, 5]},
                ...
                {"t": "Pair them with a senior engineer for a mandatory 'code refactoring sprint' disguised as a mentorship program to extract the knowledge.", "s": [0, 10, 0, 5, 0]},
                {"t": "Lock their repository access until they provide full documentation and host a team-wide knowledge-sharing session.", "s": [5, 0, 5, 0, 10]}
            ]
        },
        {
            "id": "s5",
            ...
            "opts": [
                ...
                {"t": "Refuse to provide the list and shut down the model entirely until the bias is mathematically and ethically eradicated.", "s": [5, 5, 5, 0, 10]},
                {"t": "Provide the list but manually filter out all recent maternity returnees to temporarily bandage the bias issue without delaying the bonuses.", "s": [0, 0, 0, 0, 5]},
                {"t": "Work overnight to write a post-processing algorithm that mathematically neutralizes the leave-status weights, delivering a safe list by morning.", "s": [10, 10, 10, 5, 0]}
            ]
        },
        {
            "id": "s6",
            ...
            "opts": [
                ...
                {"t": "Quietly write a reverse-script to fix the data in the middle of the night and erase the system logs to cover your tracks.", "s": [0, 10, 0, 5, 0]},
                {"t": "Fix the data quietly but leave the logs intact, hoping nobody ever runs an audit on that specific timeframe.", "s": [0, 5, 0, 0, 0]},
                {"t": "Blame the vendor API for the sudden data shift and submit an 'emergency vendor patch' to restore the tiers without taking blame.", "s": [5, 0, 5, 10, 5]}
            ]
        },
        {
            "id": "s7",
            ...
            "opts": [
                {"t": "Upload the file. The CEO's direct orders and the financial success of the M&A override a survey disclaimer.", "s": [0, 0, 0, 5, 0]},
                {"t": "Quit on the spot or refuse outright rather than violate the ethical promise of anonymity you made to the employees.", "s": [5, 5, 5, 0, 5]},
                {"t": "Strip the names but leave the raw comments and demographic tags, hoping the consultants can't reverse-engineer the identities.", "s": [0, 0, 0, 10, 0]},
                {"t": "Use an NLP script to summarize the comments into thematic, aggregated insights, refusing the raw text while still delivering the analysis.", "s": [10, 10, 10, 0, 10]}
            ]
        },
        {
            "id": "s8",
            ...
            "opts": [
                {"t": "Escalate their insubordination to leadership and request the reporting team be formally reprimanded or reassigned so you can hit your deadline.", "s": [5, 10, 0, 10, 0]},
                {"t": "Pause the predictive analytics rollout entirely to focus purely on change management and making the legacy team feel safe.", "s": [0, 0, 10, 0, 10]},
                {"t": "Carve out a safe 'shadow environment' where the legacy team can run their old checks against your new data, while you push the main platform live.", "s": [10, 0, 5, 0, 5]},
                {"t": "Surreptitiously bypass their validation checks entirely, deploying the platform and proving its accuracy to executives after the fact.", "s": [0, 5, 0, 5, 0]}
            ]
        },
        {
            "id": "s9",
            ...
            "opts": [
                ...
                {"t": "Confront your friend privately, giving them 48 hours to fix the model and claim it was a 'planned iterative update' before you escalate.", "s": [5, 5, 10, 5, 5]},
                {"t": "Keep quiet. The company is saving millions, and you don't want to destroy your friend's career over a statistical weighting issue.", "s": [0, 0, 5, 10, 0]},
                {"t": "Secretly commit a patch to their repository that lessens the penalty for flexible hours without telling them.", "s": [0, 10, 0, 0, 0]}
            ]
        },
        {
            "id": "s10",
            ...
            "opts": [
                ...
                {"t": "Flat out refuse to build it, citing the Works Council agreement and the catastrophic impact on psychological safety.", "s": [5, 5, 5, 0, 5]},
                {"t": "Build a dummy version of the dashboard with aggregated, non-individualized data to placate the COO while avoiding direct violations.", "s": [0, 0, 0, 0, 0]},
                {"t": "Present a thoroughly researched alternative: a dashboard focusing on team-level output and project velocity, persuading the COO that surveillance hurts results.", "s": [10, 10, 10, 5, 10]}
            ]
        }
    ]
